/*
 * ch_logcontext.c: CH log context
 *
...
VIR_LOG_INIT("ch.ch_logcontext");


struct _chLogContext {
    GObject parent;
...
G_DEFINE_TYPE(chLogContext, ch_log_context, G_TYPE_OBJECT);

static void
chLogContextFinalize(GObject *obj);


static void
ch_log_context_init(chLogContext *logctxt G_GNUC_UNUSED)
{
}


static void
ch_log_context_class_init(chLogContextClass *klass)
{
    GObjectClass *obj = G_OBJECT_CLASS(klass);

    obj->finalize = chLogContextFinalize;
}


static void
chLogContextFinalize(GObject *object)
{
    chLogContext *ctxt = CH_LOG_CONTEXT(object);
    ...
    G_OBJECT_CLASS(ch_log_context_parent_class)->finalize(object);
}


chLogContext *
chLogContextNew(virCHDriver *driver,
                  virDomainObj *vm,
                  const char *basename)
{
    g_autoptr(virCHDriverConfig) cfg = virCHDriverGetConfig(driver);
    chLogContext *ctxt = CH_LOG_CONTEXT(g_object_new(CH_TYPE_LOG_CONTEXT, NULL));
...
int
chLogContextWrite(chLogContext *ctxt,
                    const char *fmt, ...)
{
    va_list argptr;
...
ssize_t
chLogContextRead(chLogContext *ctxt,
                   char **msg)
{
    char *buf;
    size_t buflen;
...
/**
 * chLogContextFilter: Read and filter log for relevant messages
 ...
 */
int
chLogContextReadFiltered(chLogContext *ctxt,
                           char **msg,
                           size_t max)
{
    char *buf;
    ...
    if ((got = chLogContextRead(ctxt, &buf)) < 0)
        return -1;
...
int
chLogContextGetWriteFD(chLogContext *ctxt)
{
    return ctxt->writefd;
}
...
void
chLogContextMarkPosition(chLogContext *ctxt)
{
    if (ctxt->manager)
...
virLogManager *
chLogContextGetManager(chLogContext *ctxt)
{
    return ctxt->manager;
}
